...
██ ███ ██ ██  ███    ██ ███ ██ ██  ███         ██ ██  ██    ██ ██ 
 ███ ███  ██ ███████  ███ ███  ██ ███████     ██   ██  ██████  ██ 
\033[0m"
echo -e "    \e[31mTelegram Channel: \e[34m@SwitchVpGuide\033[0m | \e[31mTelegram Group: \e[34m@SwitchVpSupport\033[0m\n"

#sleep
echo -e "\e[32mInstalling SWITCH VP script ... \033[0m\n"
sleep 5
...
git clone https://github.com/parhamkhanmohammadi/switchbot.git /var/www/html/switchbot
sudo chown -R www-data:www-data /var/www/html/switchbot/
sudo chmod -R 755 /var/www/html/switchbot/
echo -e "\n\033[33mSWITCH VP config and script have been installed successfully\033[0m"
...
echo "Folder created successfully!"
 ...
 cd /var/www/html/
 wget -O wizwizpanel.zip https://github.com/parhamkhanmohammadi/switchbot/releases/download/10.3.1/wizwizpanel.zip
...
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/switchbot/settings/messagewizwiz.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/switchbot/settings/rewardReport.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/switchbot/settings/warnusers.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/switchbot/settings/gift2all.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/switchbot/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'wizwiz'") ]]; then
        ...
        read YOUR_DOMAIN
        printf "\e[33m[+] \e[36mWeb Port (avoid 80 and 8080, e.g. 8443): \033[0m"
        read YOUR_PORT
        if [ "$YOUR_PORT" = "" ]; then
            YOUR_PORT="8443"
        fi
        printf "\e[33m[+] \e[36mProxy (socks5://user:pass@ip:port or leave blank): \033[0m"
        read YOUR_PROXY
        if [ "$YOUR_PROXY" = "" ]; then
            YOUR_PROXY="socks5://Khodam:Parham1384@127.0.0.1:8080"
        fi
        echo " "
        ...
        file_path="/var/www/html/switchbot/baseInfo.php"
        ...
        # print file
        echo -e "<?php" >> /var/www/html/switchbot/baseInfo.php
        echo -e "error_reporting(0);" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}botToken = '${YOUR_BOT_TOKEN}';" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}dbUserName = '${dbuser}';" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}dbPassword = '${dbpass}';" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}dbName = '${dbname}';" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}botUrl = 'https://${YOUR_DOMAIN}:${YOUR_PORT}/switchbot/';" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}webPort = ;" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}admin = ${YOUR_CHAT_ID};" >> /var/www/html/switchbot/baseInfo.php
        echo -e "${ASAS}proxy = '${YOUR_PROXY}';" >> /var/www/html/switchbot/baseInfo.php
        echo -e "?>" >> /var/www/html/switchbot/baseInfo.php

        sleep 1

        curl -F "url=https://${YOUR_DOMAIN}/switchbot/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        MESSAGE="✅ The wizwiz bot has been successfully installed! @SwitchVpGuide"
        curl -s -X POST "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/sendMessage" -d chat_id="${YOUR_CHAT_ID}" -d text="$MESSAGE"
        
        
        sleep 1
        
        url="https://${YOUR_DOMAIN}/switchbot/createDB.php"
        curl $url
        
        sleep 1
        
        sudo rm -r /var/www/html/switchbot/webpanel
        sudo rm -r /var/www/html/switchbot/install
        sudo rm /var/www/html/switchbot/createDB.php
	rm /var/www/html/switchbot/updateShareConfig.php
	rm /var/www/html/switchbot/README.md
	rm /var/www/html/switchbot/README-fa.md
	rm /var/www/html/switchbot/LICENSE
	rm /var/www/html/switchbot/update.sh
	rm /var/www/html/switchbot/wizwiz.sh
	rm /var/www/html/switchbot/tempCookie.txt
	rm /var/www/html/switchbot/settings/messagewizwiz.json
